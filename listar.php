      ...
      <div class="div1">
       ...
       <div class="p1">
            ...
            <?php 
             if($op!='periodo'){
              if($op=='status'){
                 $sql = mysqli_query($conexao,"SELECT * FROM caminhao WHERE status='$stat' ORDER BY data")or die(mysqli_error($conexao));
                if(mysqli_num_rows($sql)==0){
                    echo '<script>alert("NÃO EXISTE REGISTROS COM O STATUS INFORMADO!!")</script>';
                    echo "<script>window.location='listagem.php';</script>";
                }
                else{
                  while($resp = mysqli_fetch_array($sql)){
                    $id=$resp['id']; $motorista=$resp['motorista']; $cpf=$resp['cpf']; $cnh=$resp['cnh']; $data=$resp['data'];$epi=$resp['epi']; $catcnh=$resp['catcnh'];
                    $fone=$resp['fone']; $valcnh=  converte_data($resp['val_cnh']); $transp=$resp['transp']; $placacv=$resp['placacv']; $placac=$resp['placac']; $status=$resp['status']; $saida=$resp['saida'];
                    ...
                    echo'<tr><td width="300"><strong>DATA:</strong> '.converte_data($resp['data']).'</td>'
                     ...
                     .'<tr><td><strong>Observação: </strong> '.$resp['obs'].'</td>'
              ...
              $sql = mysqli_query($conexao,"SELECT * FROM caminhao WHERE status='Aguardando' AND (motorista LIKE '%$campo%' OR cpf='$campo')")or die(mysqli_error($conexao));
                if(mysqli_num_rows($sql)==0){
                    echo '<script>alert("VEICULO/MOROTORISTA INFORMADO NÃO ESTÁ NO PÁTIO!!")</script>';
                    echo "<script>window.location='listagem.php';</script>";
                }
                else{
                  while($resp = mysqli_fetch_array($sql)){
